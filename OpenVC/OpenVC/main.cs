...
namespace OpenUtils
{
    public class main : Script
    {
        ...
        public main()
        {
            ...
            mainMenu = new UIMenu("~g~Vehicle Control", "~b~By Toxic Scams ~r~V1.2");   
        ...
        private void GameLoop(object sender, EventArgs e)
        {
            ...
            else 
            {
                ...
                foreach (Vehicle v in VehicleStore.calledVehicles)
                {
                    ...
                    {
                        v.Driver.DrivingSpeed = 50;
                    }/*else This Fixes An Issue If The Vehicle Faced A Stoped Vehicle
                    {
                        Vehicle[] vehs = World.GetNearbyVehicles(v.Position, 30.0f);
                        foreach (Vehicle veh in vehs)
                        {
                            if (veh.Exists())
                            {
                                if (veh != v && veh != playerVehicle)
                                {
                                    if (veh.Speed == 0 && !VehicleStore.IsVehicleToBeManaged(veh))
                                    {
                                        veh.Delete();
                                        v.Driver.Task.LeaveVehicle();
                                        v.Driver.Delete();
                                        Ped driver = v.CreatePedOnSeat(VehicleSeat.Driver, PedHash.Autoshop01SMM);
                                        DriveVehicleTo(driver, v, Game.Player.Character.Position, 5, CalledVehicleSpeed, CalledVehicleDrivingStyle);
        ...
        {
            int engineIndex = ControlCarEngine_CONTROL < 50 ? 0 : 2;
            int menuIndex1 = ControlMenuKey1_CONTROL < 50 ? 0 : 2;
            int menuIndex2 = ControlMenuKey2_CONTROL < 50 ? 0 : 2;
            if (Game.IsControlPressed(engineIndex, (GTA.Control) (ControlCarEngine_CONTROL)) && Game.IsControlJustReleased(menuIndex2, (GTA.Control)(ControlMenuKey2_CONTROL)))
            {
                OnKeyUp(null, new KeyEventArgs(ControlCarEngine_KEY));
            }
            if(Game.IsControlPressed(menuIndex1, (GTA.Control)(ControlMenuKey1_CONTROL)) && Game.IsControlJustReleased(menuIndex2, (GTA.Control)(ControlMenuKey2_CONTROL)))
            {
        ...
        private void OnKeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == RemoteMenu_KEY && !_menuPool.IsAnyMenuOpen())
            {
                ...
                VehicleStore.UpdateVehiclesToBeManaged();
                int index = 0;
                foreach(Vehicle v in VehicleStore.managedVehicles)
                {
                    if (v.Exists())
                    {
                        if (index == 0)
